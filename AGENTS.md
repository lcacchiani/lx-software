...
- **Executive Board tools:** board members can call tools (GitHub, board records, mail, research, AWS, security) through OpenRouter function calling (`backend/lambda/admin/board_tools.py`). Access is `off`/`read`/`propose`/`act` per tool per member, capped by a global mode; `propose` writes land in **Executive Board → Approvals**. Add a tool by registering `ToolOp`s in `board_tools.py`, adding the tool to `contracts/board-tools.json` (then run the contract sync), and covering it in `test_board_tools.py` / `test_board_t2.py` (mail in `test_board_mail.py`) with a fake behind the `HostRouter`. Cheap AWS/security reads refresh hourly (`internal: board_cache_refresh`). Kill switch: `BoardToolsEnabled` stack parameter. Details in `docs/architecture/executive-board-tools-plan.md`.
