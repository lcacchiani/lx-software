...
- **Public read-only API keys:** `/public/{finance,finance/quotes,records,fx/v2/rates}` GET routes accept an `x-api-key` header (`lxpk_<keyId>_<secret>`) validated by `backend/lambda/public_api_authorizer/` (DynamoDB `pk=APIKEY#<keyId>` + scrypt verify of the secret). Mint/list/revoke with `scripts/manage-public-api-keys.py`; docs in `docs/deployment/admin-website.md`. Write routes stay Cognito-JWT only.
