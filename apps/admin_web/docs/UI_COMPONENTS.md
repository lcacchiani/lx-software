...
| `BoardToolsCard` | Tools & permissions: kill switch, global mode, the tool × member level matrix (cells show `→ effective` when the global mode caps them), per-tool operation list, the **Recipient allow-list** textarea (one address, `@domain`, or E.164 phone per line), **Meta ads spend caps** (daily / monthly) with spend-versus-cap copy from `adsSpend`, configured hints on research / finance / product / meta / stores rows, and the collapsible call log. Keyed on the saved config like the other editors. |
